...
fli <- read_csv("data/csv/full_lemma_in.csv") %>%
  # create an id for election periods for later merging
  # in case someone switches party
  mutate(id_obdobi = case_when(str_detect(doc, "2010ps") ~ 170,
                               str_detect(doc, "PS 2013") ~ 171,
                               str_detect(doc, "PS 2017") ~ 172))
```


# Match speaker with party
List of MPs
```{r list of MPs and partisanship}
# load psp metadata
