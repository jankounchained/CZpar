...
calculate_ntr <- function(doc_subset_path, w) {
  ...
  ntr_output = tibble(doc_id = id$doc_id) %>%
    mutate(
  ...
  ntr_output %>%
    write_csv(paste0("data/ntr", "/", str_extract(doc_subset_path, "\\d"), "_from_5k.csv"))
...
sub_path <- list.files(path = "data/ntr/", pattern = "_to.csv", full.names = T)

plan(multiprocess)
future.apply::future_lapply(sub_path, calculate_ntr, w = 5000)
